#!/usr/bin/env python3
"""
Build processed_ton_iot_eval.pkl from processed_ton_iot.pkl.
Converts ALL rows — no sampling, no row limit.
Run once after preprocess_ton_iot.py.
"""
...
PKL_IN  = Path(DATA_DIR) / "processed_ton_iot.pkl"
PKL_OUT = Path(DATA_DIR) / "processed_ton_iot_eval.pkl"
...
def normalize_row(r: Dict[str, Any]) -> Dict[str, Any]:
    ...
    x["scenario"]       = clean_str(x.get("scenario", "ton_iot"))
...
def main() -> int:
    print(f"\n[create_eval TON_IoT]  {PKL_IN}  ->  {PKL_OUT}")
    if not PKL_IN.exists():
        print("ERROR: processed_ton_iot.pkl not found.")
        print("Run: python scripts/preprocess_ton_iot.py")
        return 1
    ...
    print(f"[create_eval TON_IoT] {len(rows):,} rows normalized")
    ...
    print(f"[create_eval TON_IoT] rows={len(rows):,}  benign={benign:,}  urgent={urgent:,}")
    print(f"[create_eval TON_IoT] Saved -> {PKL_OUT}")
    return 0
