#!/usr/bin/env python3
"""
Lightweight IoT-23 preprocessing for eval_engine.py
Input:  data/IoT_23/iot23_250k.csv
Output: data/processed_iot23.pkl
"""
...
INPUT    = DATA_DIR / "IoT_23" / "iot23_250k.csv"
OUTPUT   = DATA_DIR / "processed_iot23.pkl"

NUM_DEVICES = 20
CHUNK_SIZE  = 100_000
BENIGN_KEYS = {"benign", "normal", "0", "false"}
MAL_KEYS    = {"malicious", "attack", "1", "true"}
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rename_map = {
        "ts": "timestamp", "id_orig_h": "src_ip", "id_orig_p": "src_port",
...
def build_reading(raw: Dict[str, Any], row_index: int, source_row: int, source_file: str) -> Dict[str, Any]:
    ...
    detailed_label = clean_str(raw.get("detailed_label") or raw.get("label") or "").lower()
    ...
    ts_raw = raw.get("timestamp", None)
    timestamp = (
        clean_float(ts_raw, float(source_row))
        if ts_raw not in (None, "", "-", "nan", "none")
        else float(source_row)
    )
    ...
    reading: Dict[str, Any] = {
        ...
        "scenario":       clean_str(raw.get("scenario", "iot-23")),
    }
...
def main() -> int:
    ...
    if not rows:
        raise RuntimeError("No IoT-23 rows were loaded.")
    ...
    print(f"\n[IoT-23] rows={len(rows):,}  benign={benign:,}  urgent={urgent:,}")
    print(f"[IoT-23] device_ids={devices}")
    print(f"[IoT-23] Saved -> {OUTPUT}")
    return 0
