#!/usr/bin/env python3
"""
Build processed_nbaiot_eval.pkl from processed_nbaiot.pkl.
Converts ALL rows — no sampling, no row limit.
Run once after preprocess_nbaiot.py.
"""
...
PKL_IN  = Path(DATA_DIR) / "processed_nbaiot.pkl"
PKL_OUT = Path(DATA_DIR) / "processed_nbaiot_eval.pkl"
...
def normalize_row(r: Dict[str, Any]) -> Dict[str, Any]:
    ...
    x["timestamp"]      = clean_float(x.get("timestamp", float(x["row_index"])))
    x["label"]          = clean_str(x.get("label", "malicious")).lower()
    x["detailed_label"] = clean_str(x.get("detailed_label", x["label"])).lower()
    x["source_file"]    = clean_str(x.get("source_file", ""))
    x["scenario"]       = clean_str(x.get("scenario", "nbaiot"))
...
def main() -> int:
    print(f"\n[create_eval N-BaIoT]  {PKL_IN}  ->  {PKL_OUT}")
    if not PKL_IN.exists():
        print("ERROR: processed_nbaiot.pkl not found.")
        print("Run: python scripts/preprocess_nbaiot.py")
        return 1
    ...
    print(f"[create_eval N-BaIoT] {len(rows):,} rows normalized")
    ...
    print(f"[create_eval N-BaIoT] rows={len(rows):,}  benign={benign:,}  urgent={urgent:,}")
    print(f"[create_eval N-BaIoT] Saved -> {PKL_OUT}")
    return 0
